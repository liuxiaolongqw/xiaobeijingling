...
;(function (window, document, Math) {
	...
	//保存jroll对象
	function saveJRoll(j) {
		var id = j.scroller.getAttribute("jroll-id");

		if (!id) {
			id = "jroll_" + Math.random().toString().substr(2, 8);
			j.id = id;
			j.scroller.setAttribute("jroll-id", id);
		}
		jrollMap[id] = j;
	}
	...
	JRoll.prototype = {
		...
		destroy : function() {
			...
			me.disable();
			me.scroller.removeAttribute("style");
			me.prototype = null;
			for (var i in me) {
